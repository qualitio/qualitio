...
class Test03Gmail(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium("localhost", 4444, "*chrome", "http://127.0.0.1:8001/")
        self.selenium.start()
    ...
    def test_03_gmail(self):
        ...
        for i in range(60):
            try:
                if sel.is_element_present("//div[@id='submit-panel']/div[1]/a/span"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.failUnless(sel.is_element_present("//div[@id='submit-panel']/div[1]/a/span"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        sel.click("//div[@id='submit-panel']/div[1]/a/span")
        sel.wait_for_page_to_load("30000")
        for i in range(60):
            try:
        ...
        for i in range(60):
            try:
                if sel.is_text_present("JackPack"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.failUnless(sel.is_text_present("JackPack"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_element_present("link=Log out"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        sel.click("link=Log out")
...
class Test10ModreqDepend(BaseSeleniumTestCase):
    ...
    def test_10_modreq_depend(self):
        ...
        # <tr>
        # 	<td>waitForText</td>
        # 	<td>css=div#application-view-header h1</td>
        # 	<td>requirement: new requirement 7</td>
        # </tr>
        # <tr>
        # 	<td>click</td>
        # 	<td>link=details</td>
...
class Test14TestcasesVerify(BaseSeleniumTestCase):
    ...
    def test_14_testcases_verify(self):
        ...
        try: self.failUnless(sel.is_element_present("//form[@id='testcases_connect_form']/div[3]/div/div/div[1]/div[1]/div/table/thead/tr/th[1]/input"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_element_present("css=a#remove-testcases-button span"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_element_present("css=a#add-testcases-button span"))
