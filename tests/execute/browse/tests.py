...
class Test47ExecTestrunClone(BaseSeleniumTestCase):
    
    def test_47_exec_testrun_clone(self):
        self.login() 
        sel = self.selenium
        sel.open("/project/meego/require/#requirement/1/details/")
        self.assertEqual("test@qualitio :: require", sel.get_title())
        ...
        sel.click("link=Test run testowy")
        time.sleep(1)
        for i in range(60):
            try:
                if "Clone" == sel.get_value("name=3"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        sel.click("name=3")
        ...
        for i in range(60):
            try:
                if sel.is_text_present("Test run testowy (copy)"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        sel.click("link=Test run testowy (copy)")
        for i in range(60):
            try:
                if sel.is_text_present("test run: Test run testowy (copy)"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        for i in range(60):
        ...
        try: self.failUnless(sel.is_text_present("/TestRun directory/Test run testowy (copy)"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        ...
        for i in range(60):
            try:
                if sel.is_text_present("exact:4: /TestRun directory/Test run testowy (copy)"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        self.failUnless(sel.is_text_present("exact:4: /TestRun directory/Test run testowy (copy)"))
        self.failUnless(sel.is_text_present("exact:3: /TestRun directory/Test run testowy"))
        sel.click("css=input[type=\"submit\"]")
        sel.wait_for_page_to_load("30000")
...
class Test49ExecTestrunPassrate(BaseSeleniumTestCase):
    
    def test_49_exec_testrun_passrate(self):
        self.login() 
        sel = self.selenium
        sel.open("/project/meego/require/#requirement/1/details/")
        self.assertEqual("test@qualitio :: require", sel.get_title())
        ...
        try: self.failUnless(sel.is_text_present("100.0 %"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.assertEqual("100.0 %", sel.get_text("css=div.value"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_element_present("css=div.element.PASSED[@style=\"width: 100%; background: none repeat scroll 0% 0% rgb(136, 187, 102);\"]"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        for i in range(60):
            try:
                if sel.is_element_present("//tr[@style=\"background: none repeat scroll 0% 0% rgb(136, 187, 102);\"]"): break
        ...
        try: self.failUnless(sel.is_text_present("0.0 %"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.assertEqual("", sel.get_text("css=div.element.FAILED"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        try: self.failUnless(sel.is_element_present("css=div.element.FAILED[@style=\"width: 100%; background: none repeat scroll 0% 0% rgb(255, 0, 0);\"]"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        for i in range(60):
            try:
                if sel.is_element_present("//tr[@style=\"background: none repeat scroll 0% 0% rgb(255, 0, 0);\"]"): break
